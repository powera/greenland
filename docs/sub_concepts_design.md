...
seasons of MLB teams (2004 Boston Red Sox season), cities in Turkey, individual
bird species -- can be tracked in the concepts database without becoming part
of the main encyclopedia. Sub-concepts live in their own table but **share the
Wikidata Q-id index** (`concept_wikidata_index`) with main concepts. No body
generation for these entries is planned yet; the immediate value is being able
to **filter them out** of the main concept list, the Voverukas red-link
rankings, and the generation pipelines, while keeping a durable record
(slug + Q-id + category) of each topic.
...
class SubConcept(Base):
    ...
    main concept (e.g. a single chess opening, one team's season, one city,
    one bird species). Exists primarily so these topics can be filtered out
    of main-encyclopedia listings, rankings, and generation.
    """
...
- **A `tier` column on `concepts`** (previous revision of this design) -
  ...
  thousands of body-less rows (there are far more bird species and city
  Q-ids than main topics).
...
Voveraite is the natural intake for hand-collected Q-id lists ("all cities in
...
3. **Category granularity.** Is `city` one category, or should intake batches
   carry a finer note ("cities in Turkey") in `notes`? Recommendation: keep
   the vocabulary coarse and put the batch context in `notes`, so the
   hardcoded list stays short.
