...
async def text_completion(prompt):
    ...
    with torch.no_grad():
      ...
      output = await loop.run_in_executor(None, lambda: model.generate(
        input_ids,
        max_length=192,
...
def text_completion_interactive(prompt):
    ...
    with torch.no_grad():
      ...
      for _ in range(32):
        ...
        # Use top_k
        top_k_values, top_k_indices = torch.topk(torch.log_softmax(logits[:, -1, :], dim=-1), 12, dim=-1)
...
def text_chat_interactive(prompt):
    ...
    with torch.no_grad():
      ...
      for _ in range(64):
        ...
        # Use top_k
        top_k_values, top_k_indices = torch.topk(torch.log_softmax(logits[:, -1, :], dim=-1), 18, dim=-1)
        top_k_logprobs = torch.exp(top_k_values).squeeze().tolist()
        top_k_words = tokenizer.convert_ids_to_tokens(top_k_indices.squeeze().tolist())
