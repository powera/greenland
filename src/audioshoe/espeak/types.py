...
class EspeakVoice(Enum):
    ...
    def get_regular_voices_for_language(cls, language_code: str) -> list["EspeakVoice"]:
        """Get only regular eSpeak voices for a specific language."""
        return [voice for voice in cls if voice.language_code == language_code]


# Default voices for each language (2F/2M per language)
...
# Recommended voices (same as default)
RECOMMENDED_VOICES = DEFAULT_ESPEAK_VOICES
