    ...
    def _generate_with_llm(self, **kwargs) -> Iterator[BenchmarkQuestion]:
        ...
        while len(exhausted_categories) < len(category_generators):
            for category_name, generator_func, max_items in category_generators:
                ...
                if category_name == "capitals" and len(self._used_countries) >= len(COUNTRIES) * 0.9:
                    exhausted_categories.add(category_name)
                    logger.info(f"Used most countries ({len(self._used_countries)}/{len(COUNTRIES)}), marking capitals as exhausted")
                elif category_name == "mountains" and len(self._used_mountains) >= len(MOUNTAINS) * 0.9:
                    exhausted_categories.add(category_name)
                    logger.info(f"Used most mountains ({len(self._used_mountains)}/{len(MOUNTAINS)}), marking mountains as exhausted")
                elif category_name == "rivers" and len(self._used_rivers) >= len(RIVERS) * 0.9:
                    exhausted_categories.add(category_name)
                    logger.info(f"Used most rivers ({len(self._used_rivers)}/{len(RIVERS)}), marking rivers as exhausted")
                elif category_name == "cities" and len(self._used_cities) >= len(CITIES) * 0.9:
                    exhausted_categories.add(category_name)
                    logger.info(f"Used most cities ({len(self._used_cities)}/{len(CITIES)}), marking cities as exhausted")
        
        # If we've exhausted all the predefined entries, switch to freestyle generation
        logger.info("All specialized categories exhausted, switching to freestyle geography question generation")
    ...
    def _generate_freestyle_questions(self) -> Iterator[BenchmarkQuestion]:
        ...
        schema = {
            ...
            "properties": {
                "question": {
                    "type": "string",
                    "description": "The geography question text"
                },
                "correct_answer": {
                    "type": "string",
            ...
            "required": ["question", "correct_answer", "incorrect_answers", "category", "difficulty"]
        }
