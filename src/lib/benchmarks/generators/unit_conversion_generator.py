...
class UnitConversionGenerator(BenchmarkGenerator):
    ...
    def _generate_conversion_question(
        self, conversion: Dict, value: Optional[float] = None
    ) -> BenchmarkQuestion:
        ...
        if conversion["from_unit"] in ["fahrenheit", "celsius"] or conversion["to_unit"] in [
            "fahrenheit",
            "celsius",
        ]:
        ...
        eval_criteria = EvaluationCriteria(exact_match=False, tolerance=tolerance)
    ...
    def _generate_from_file(self, **kwargs) -> Iterator[BenchmarkQuestion]:
        ...
        try:
            ...
            logger.info(
                "No questions.json found, using conversion definitions to generate questions"
            )
