...
4. Use simple, clear language"""
    ...
    def _generate_with_llm(self, **kwargs) -> Iterator[BenchmarkQuestion]:
        ...
        while len(used_words) < len(words):  # Stop when all words have been used as correct answers
            ...
            try:
                response = self.get_llm_question(prompt=prompt, schema=self.schema)
                ...
                logging.getLogger(__name__).error(
                    f"Error generating definition for '{correct}': {e}"
                )
                continue
        ...
        if len(used_words) >= len(words) - 10:
            ...
            for difficulty in difficulty_levels:
                for category in categories:
                    ...
                    try:
                        response = self.get_llm_question(prompt=prompt, schema=full_question_schema)
