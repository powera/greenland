...
def main():
    ...
    run_parser.add_argument(
        "exemplar_id", nargs="?", help="ID of the exemplar to run (omit for all)"
    )
    ...
    report_parser.add_argument(
        "exemplar_id", nargs="?", help="ID of the exemplar to generate a report for (omit for all)"
    )
    ...
    if args.command == "list":
        ...
        if args.exemplar_id:
            ...
            # Run all exemplars with a specific model
            model = (
                args.models[0] if args.models else None
            )  # Will use first database model by default
