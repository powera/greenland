        ...
        properties={
            "analysis": SchemaProperty(
                ...
                properties={
                    ...
                    "makes_sense": SchemaProperty("boolean", "Whether this combination makes logical sense")
                }
            ),
...
def run_sentence_generation_demo(models: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    for i in range(3):
        ...
        if pattern:
            ...
            print(f"Pattern {i+1}:")
            print(f"  Components: {pattern['subject']['key']} + {pattern['verb']['key']} + {pattern['object']['key']} ({pattern['tense']})")
    ...
    if llm_client:
        ...
        for i, test_case in enumerate(test_cases):
            ...
            pattern = {
                "subject": {"key": test_case["subject"], "data": SAMPLE_WORD_MATRICES["subjects"][test_case["subject"]]},
                "verb": {"key": test_case["verb"], "data": SAMPLE_WORD_MATRICES["verbs"][test_case["verb"]]},
                "object": {"key": test_case["object"], "data": SAMPLE_WORD_MATRICES["foods"].get(test_case["object"]) or SAMPLE_WORD_MATRICES["objects"].get(test_case["object"])},
