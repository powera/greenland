...
class SentenceGenerator:
    ...
    def __init__(
        self,
        word_matrices: Dict[str, Dict[str, Dict]],
    ...
    def create_sentence_pattern(self, pattern_type: str = "SVO") -> Optional[Dict[str, Any]]:
        ...
        # Debug logging
        logger.debug(
            f"Verb: {verb_key}, Compatible subjects: {len(compatible_subjects)}, Compatible objects: {len(compatible_objects)}"
        )
    ...
    def pattern_to_sentences(
        self, pattern: Dict[str, Any], target_languages: List[str]
    ) -> Dict[str, str]:
        """
        ...
        schema = Schema(
            ...
            f"Generated {target_language} sentence with proper grammar",
            {
                "makes_sense": SchemaProperty(
                    "boolean", "Whether the sentence combination makes logical sense"
                ),
                "reason": SchemaProperty(
                    "string", "Explanation if the sentence doesn't make sense", required=False
                ),
                "english": SchemaProperty("string", "Natural English sentence", required=False),
                "target_sentence": SchemaProperty(
                    "string", f"Grammatically correct {target_language} sentence", required=False
                ),
                "adjective_used": SchemaProperty(
                    "string", "Adjective that was included, or null", required=False
                ),
            },
        )

        try:
            response = self.llm_client.generate_chat(prompt=prompt, model=model, json_schema=schema)
    ...
    def _get_compatible_words(
        self, word_type: str, verb_data: Dict[str, Any]
    ) -> List[Tuple[str, Dict]]:
    ...
    def _build_target_language_sentence(self, pattern: Dict[str, Any], lang_code: str) -> str:
        ...
        if lang_code in self.grammar_rules:
            ...
            if "verb_conjugation" in rules:
                verb_word = self._apply_conjugation(
                    verb_word, pattern["tense"], subject_word, rules["verb_conjugation"]
                )
        ...
        if "adjective" in pattern:
            ...
            if lang_code in self.grammar_rules and "cases" in self.grammar_rules[lang_code]:
                adj_word = self._apply_case(
                    adj_word, "accusative", self.grammar_rules[lang_code]["cases"]
                )
    ...
    def _apply_conjugation(
        self, verb: str, tense: str, subject: str, conjugation_rules: Dict[str, Any]
    ) -> str:
