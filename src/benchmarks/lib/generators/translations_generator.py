...
# Path to the exported translations file (generated by export_translations.py)
_TRANSLATIONS_EXPORT_PATH = os.path.join(
...
class TranslationGenerator(BenchmarkGenerator):
    ...
    def _generate_from_file(self, **kwargs: Any) -> Iterator[BenchmarkQuestion]:
        """
        Generate translation questions from the exported translations JSON file.

        Loads translations_export.json (produced by export_translations.py),
        filters for entries that have both origin and target languages, and
        yields BenchmarkQuestion objects.

        Yields:
            BenchmarkQuestion objects one at a time
        """
        if not os.path.exists(_TRANSLATIONS_EXPORT_PATH):
            logger.debug("Translations export file not found at %s", _TRANSLATIONS_EXPORT_PATH)
            return

        try:
            with open(_TRANSLATIONS_EXPORT_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
        except (json.JSONDecodeError, OSError) as e:
            logger.warning("Failed to load translations export: %s", e)
            return

        file_entries = data.get("entries", [])
        if not file_entries:
            logger.debug("No entries in translations export file")
            return

        # Filter entries that have both origin and target language translations
        valid_entries: List[Dict[str, Any]] = []
        for entry in file_entries:
            translations = entry.get("translations", {})
            # For English origin, the word is in "en" key directly
            if self.origin_lang == "en":
                origin_word = entry.get("en", "")
            else:
                origin_word = translations.get(self.origin_lang, "")

            if self.target_lang == "en":
                target_word = entry.get("en", "")
            else:
                target_word = translations.get(self.target_lang, "")

            if origin_word and target_word:
                valid_entries.append({"origin_word": origin_word, "target_word": target_word})

        if not valid_entries:
            logger.debug(
                "No entries with both %s and %s in export file",
                self.origin_lang,
                self.target_lang,
            )
            return
        ...
        logger.info(
            "Loaded %d translation pairs from export file for %s -> %s",
            len(valid_entries),
            self.origin_lang,
            self.target_lang,
        )
        ...
        for pair in valid_entries:
            yield self._create_question(pair["origin_word"], pair["target_word"])
