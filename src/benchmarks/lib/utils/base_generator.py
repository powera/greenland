...
class BenchmarkGenerator:
    ...
    def __init__(self, metadata: BenchmarkMetadata, session=None, auto_validate: bool = False):
        ...
        self._file_generator: Optional[Iterator[BenchmarkQuestion]] = None
        self._local_generator: Optional[Iterator[BenchmarkQuestion]] = None
        self._llm_generator: Optional[Iterator[BenchmarkQuestion]] = None
        self._combined_generator: Optional[Iterator[BenchmarkQuestion]] = None
    ...
    def load_json_file(self, filename: str, subdir: Optional[str] = None) -> Any:
        ...
        if subdir:
            file_path = os.path.join(
                benchmark_constants.BENCHMARK_DATA_DIR, self.metadata.code, subdir, filename
            )
        else:
            file_path = os.path.join(
                benchmark_constants.BENCHMARK_DATA_DIR, self.metadata.code, filename
            )
    ...
    def get_llm_question(
        ...
        response = unified_client.generate_chat(
            prompt=prompt, model=model, json_schema=schema, context=ctx
        )
        ...
        if strategy:
            ...
            try:
                ...
                if not enabled_strategies:
                    ...
                    raise ValueError(
                        f"All enabled generation strategies are exhausted: {', '.join(enabled_strategies)}"
                    )
    ...
    def batch_save_questions(
        self, questions: List[BenchmarkQuestion], id_prefix: Optional[str] = None
    ) -> List[str]:
        """
    ...
    def load_to_database(
        self, num_questions: int = 40, strategy: Optional[str] = None
    ) -> List[str]:
        """
        ...
        if strategy:
            ...
            questions = list(
                itertools.islice(self.generate_questions_iter(num_questions), num_questions)
            )
