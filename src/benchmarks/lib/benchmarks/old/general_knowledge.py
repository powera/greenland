...
from benchmarks.lib.benchmarks.base import BenchmarkResult, BenchmarkRunner
        ...
        for question in questions:
            ...
            try:
                free_response, _, perf = unified_client.generate_chat(
                    prompt=info["context"], model=self.remote_model, context=self.context
                )
                ...
                debug_info = (
                    None
                    if is_correct
                    else {"response": free_response, "expected": info["continuation"]}
                )
