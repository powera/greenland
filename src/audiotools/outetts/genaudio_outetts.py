...
def process_file(interface, file_path, output_dir, speaker_name=None, force=False, output_format="wav", debug_mode=False):
    """
...
def process_lithuanian_batch(interface, file_path, output_dir, force=False, speaker_name="ash", output_format="wav", debug_mode=False):
    """
...
def process_json_file(interface, json_path, output_dir, force=False, speaker_name="ash", output_format="wav", items_key=None, debug_mode=False):
    """
    ...
    for i, item in enumerate(items, 1):
        ...
        # Skip if target file exists and not forcing overwrite
        if target_file.exists() and not force:
            print(f"[{i}/{total_count}] Skipping {filename}: file already exists in {output_format} format")
            success_count += 1  # Count as success since file exists
            continue
        ...
        if generate_lithuanian_audio(interface, lithuanian_text, wav_file, speaker_name, debug_mode):
            success_count += 1
            # Add a small delay between generations to avoid overloading
            time.sleep(0.5)
...
def main():
    ...
    input_group.add_argument("--json-file", type=str, help="Path to a JSON file with sentences/phrases (must include 'filename' field)")
    ...
    parser.add_argument("--items-key", type=str, help="Key in JSON containing items (default: auto-detect)")
    ...
    try:
        ...
        if args.text:
            ...
            if target_path.exists() and not args.force:
                ...
                if need_conversion and not output_path.endswith(".wav"):
                    ...
                    if wav_path.exists() and not args.force:
                        ...
                        generate_audio(interface, args.text, str(wav_path), args.speaker, args.debug)
            ...
            process_file(interface, args.file, output_path, args.speaker, args.force, args.format, args.debug)
            ...
            if target_path.exists() and not args.force:
                ...
                if need_conversion and not str(output_path).endswith(".wav"):
                    ...
                    if wav_path.exists() and not args.force:
                        ...
                        generate_lithuanian_audio(interface, args.lithuanian, str(wav_path), args.lithuanian_speaker, args.debug)
                    
                    # Convert to desired format if target doesn't exist or force is True
                    if not target_path.exists() or args.force:
                        convert_audio(wav_path, args.format, args.quality, delete_original)
                else:
                    generate_lithuanian_audio(interface, args.lithuanian, str(output_path), args.lithuanian_speaker, args.debug)
