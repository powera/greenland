...
from langtools.pt.llm_forms import query_portuguese_verb_conjugations
...
class TestPortugueseMechanicalLlmForms(unittest.TestCase):
    def test_uses_mechanical_conjugation(self) -> None:
        ...
        with (
            patch("langtools.pt.llm_forms.get_translation", return_value="falar"),
            patch("langtools.pt.llm_forms.query_forms") as mock_query_forms,
            patch("langtools.pt.llm_forms.linguistic_db.log_query") as mock_log_query,
        ):
            get_session = cast(Callable[[], Any], lambda: _FakeSession(lemma))
            forms, ok = query_portuguese_verb_conjugations(client, 1, get_session)

        self.assertTrue(ok)
        self.assertEqual(forms["1s_present"], "falo")
        self.assertEqual(forms["3s_past"], "falou")
