...
from langtools.nl.llm_forms import query_dutch_verb_conjugations
...
class TestDutchMechanicalLlmForms(unittest.TestCase):
    def test_uses_mechanical_conjugation(self) -> None:
        ...
        with (
            patch("langtools.nl.llm_forms.get_translation", return_value="maken"),
            patch("langtools.nl.llm_forms.query_forms") as mock_query_forms,
            patch("langtools.nl.llm_forms.linguistic_db.log_query") as mock_log_query,
        ):
            get_session = cast(Callable[[], Any], lambda: _FakeSession(lemma))
            forms, ok = query_dutch_verb_conjugations(client, 1, get_session)

        self.assertTrue(ok)
        self.assertEqual(forms["3s_present"], "maakt")
        self.assertEqual(forms["1p_future"], "zullen maken")
