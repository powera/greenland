...
The bug these guard against: :mod:`storage.connection_pool` built its engine
without the pragma listener, so it ran at SQLite's default ``busy_timeout=0``.
...
class SqlitePragmaTests(unittest.TestCase):
    ...
    def _assert_configured(self, engine: Any, label: str) -> None:
        ...
        # The regression: this read back 0 on the pooled engine.
    ...
    def test_connection_pool_engine_sets_pragmas(self) -> None:
        pool = ConnectionPool()
        self._assert_configured(pool.get_engine(self.db_path), "connection_pool")
