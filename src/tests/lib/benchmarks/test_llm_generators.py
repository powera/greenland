...
class LLMResponseCache:
    ...
    def load_cache(self):
        ...
        if os.path.exists(self.cache_file_path):
            try:
                ...
                logger.info(
                    f"Loaded {len(self.cache)} cached LLM responses from {self.cache_file_path}"
                )
    ...
    def compute_key(
        self, prompt: str, model: str, schema: Optional[Dict] = None, context: Optional[str] = None
    ) -> str:
...
class GeneratorTestCase(unittest.TestCase):
    ...
    def _setup_llm_stubs(self):
        ...
        self.mock_random_sample.side_effect = lambda population, k: population[
            :k
    ...
    def _mock_generate_chat(
        self, prompt, model, brief=False, json_schema=None, context=None, timeout=None
    ):
    ...
    def test_generate_llm_question(self):
        ...
        if self.record_mode:
            ...
            def record_wrapper(prompt, model, **kwargs):
                ...
                llm_cache.put(
                    prompt, model, cached_data, kwargs.get("json_schema"), kwargs.get("context")
                )
...
def get_test_suite():
    ...
    for benchmark_code in get_all_benchmark_codes():
        ...
        test_class = type(test_class_name, (GeneratorTestCase,), {"benchmark_code": benchmark_code})
...
def parse_args():
    ...
    cache_group.add_argument(
        "--stub-llm", action="store_true", help="Stub out LLM calls using cached responses"
    )
    cache_group.add_argument(
        "--record",
        action="store_true",
    ...
    parser.add_argument(
        "--cache-file",
        default="llm_responses_cache.json",
        help="Path to the cache file (default: llm_responses_cache.json)",
    )

    # Random seed
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for deterministic tests (default: 42)"
    )
...
def run_tests(
    stub_llm=False, record_mode=False, cache_file="llm_responses_cache.json", random_seed=42
):
