...
class TestBenchmarkGenerators(unittest.TestCase):
    ...
    def test_spell_check_generator_from_file(self, mock_listdir):
        ...
        metadata = BenchmarkMetadata(
            code="0015_spell_check", name="Spell Check", description="Test"
        )
    ...
    def test_letter_count_generator_local(self):
        ...
        # Create metadata
        metadata = BenchmarkMetadata(
            code="0012_letter_count", name="Letter Count", description="Test"
        )
    ...
    def test_definitions_generator_with_wordlist(self, mock_path_join, mock_open):
        ...
        # Create metadata
        metadata = BenchmarkMetadata(
            code="0020_definitions", name="Definitions", description="Test"
        )

        # Create generator with mock load_text_file method
        with patch.object(
            DefinitionsGenerator, "load_text_file", return_value=["apple", "banana", "cherry"]
        ):
    ...
    def test_save_question(self):
        ...
        with patch(
            ...
            mock_insert_question.assert_called_once_with(
                self.mock_session, "test_benchmark:custom_id", "test_benchmark", ANY
            )
