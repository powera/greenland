...
def write_report(
    ...
    *,
    ...
    lines.append("-" * 72)
    lines.append("")
    lines.append("These are placed by hand for two different reasons.")
    lines.append("")
    lines.append("'be' and 'have': their English counts are mostly auxiliary uses --")
    lines.append("'I have seen', 'I was going' -- which is periphrastic tense, not the")
    lines.append("verb, and other languages do not build tenses that way. They are in")
    lines.append("the database with their content senses ('to exist', 'to possess').")
    lines.append("")
    lines.append("'like': the measurement gets it wrong, scoring 4.3x on animal from")
    lines.append("'animals like wolves' -- the preposition. It is one of the most")
    lines.append("broadly combinable verbs there is and belongs early regardless.")
    lines.append("")
    lines.append("'do' is deliberately absent, here and from the database. Unlike 'be'")
    lines.append("and 'have' it has no teachable content sense: its uses are auxiliary,")
    lines.append("emphatic ('I do like it') and pro-verb ('she did too'), none of which")
    lines.append("survives a word-for-word gloss or maps to one verb in another")
    lines.append("language. That is a decision, not an oversight.")
    lines.append("")
