...
"""Swedish language form generation."""
...
NOUN_FORM_MAPPING: Dict[str, GrammaticalForm] = {
    "singular": GrammaticalForm.NOUN_SV_SINGULAR,
    "plural": GrammaticalForm.NOUN_SV_PLURAL,
}
...
def query_swedish_noun_forms(
    client: UnifiedLLMClient, lemma_id: int, get_session_func: Callable[[], Session]
) -> Tuple[Dict[str, str], bool]:
    """Query LLM for Swedish noun forms (singular and plural)."""
    session = get_session_func()
    lemma = session.query(linguistic_db.Lemma).filter(linguistic_db.Lemma.id == lemma_id).first()

    # Get Swedish translation from lemma_translations table
    swedish_translation = (
        session.query(linguistic_db.LemmaTranslation)
        .filter(
            linguistic_db.LemmaTranslation.lemma_id == lemma_id,
            linguistic_db.LemmaTranslation.language_code == "sv",
        )
        .first()
    )

    if not lemma or not swedish_translation or lemma.pos_type.lower() != "noun":
        logger.error(f"Invalid lemma for Swedish noun forms: {lemma_id}")
        return {}, False

    noun, english_noun, definition, pos_subtype = (
        swedish_translation.translation,
        lemma.lemma_text,
        lemma.definition_text,
        lemma.pos_subtype,
    )
    fields = ["singular", "plural"]
    form_properties = {f: SchemaProperty("string", f"Swedish {f}") for f in fields}

    schema = Schema(
        name="SwedishNounForms",
        description="Swedish noun forms",
        properties={
    ...
    try:
        context = util.prompt_loader.get_context("language_forms", "swedish/noun")
        prompt = util.prompt_loader.get_prompt("language_forms", "swedish/noun").format(
            noun=noun,
        ...
        logger.error(f"Error querying Swedish noun forms for '{noun}': {e}")
        return {}, False
...
def query_swedish_verb_conjugations(
    ...
    if not lemma or not swedish_translation or lemma.pos_type.lower() != "verb":
        logger.error(f"Invalid lemma for Swedish verb conjugations: {lemma_id}")
        return {}, False
    ...
    try:
        context = util.prompt_loader.get_context("language_forms", "swedish/verb")
        prompt = util.prompt_loader.get_prompt("language_forms", "swedish/verb").format(
            verb=verb,
        ...
        logger.error(f"Error querying Swedish verb conjugations for '{verb}': {e}")
        return {}, False
