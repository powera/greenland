"""Tamil-specific type definitions for linguistic forms."""
...
class NounDeclension:
    """Tamil noun declension results.

    Tamil nouns have singular and plural forms. Tamil distinguishes
    rational (human) and irrational (non-human) nouns but does not have
    grammatical gender on nouns in the European sense.
    """
...
class VerbConjugation:
    """Tamil verb conjugation results.

    Tamil verbs conjugate by person, number, and gender in 3rd person.
    For simplicity we use the standard 6-person schema (masculine 3rd person).
    """

    word: str  # The infinitive/dictionary form
    forms: Dict[str, str] = field(default_factory=dict)
    alternatives: Dict[str, List[str]] = field(default_factory=dict)
    raw_template: Optional[str] = None
    confidence: float = 1.0
