...
def query_thai_noun_forms(
    ...
    """Query LLM for Thai noun forms (singular and plural)."""
    ...
    thai_translation = (
        ...
        .filter(
            ...
            linguistic_db.LemmaTranslation.language_code == "th",
        )
        .first()
    )

    if not lemma or not thai_translation or lemma.pos_type.lower() != "noun":
        logger.error(f"Invalid lemma for Thai noun forms: {lemma_id}")
        return {}, False

    noun, english_noun, definition, pos_subtype = (
        thai_translation.translation,
        lemma.lemma_text,
        lemma.definition_text,
        lemma.pos_subtype,
    )
    fields = ["singular", "plural"]
    form_properties = {f: SchemaProperty("string", f"Thai {f}") for f in fields}

    schema = Schema(
        name="ThaiNounForms",
        description="Thai noun forms",
        properties={
    ...
    try:
        context = util.prompt_loader.get_context("language_forms", "thai/noun")
        prompt = util.prompt_loader.get_prompt("language_forms", "thai/noun").format(
            noun=noun,
        ...
        logger.error(f"Error querying Thai noun forms for '{noun}': {e}")
        return {}, False
...
def query_thai_verb_forms(
    ...
    """Query LLM for Thai verb forms (present, past, future constructions)."""
    session = get_session_func()
    lemma = session.query(linguistic_db.Lemma).filter(linguistic_db.Lemma.id == lemma_id).first()
    thai_translation = get_translation(session, lemma, "th") if lemma else None

    if not lemma or not thai_translation or lemma.pos_type.lower() != "verb":
        logger.error(f"Invalid lemma for Thai verb forms: {lemma_id}")
        return {}, False
    ...
    form_properties = {f: SchemaProperty("string", f"Thai {f} form") for f in fields}

    schema = Schema(
        name="ThaiVerbForms",
        description="Thai verb forms",
        properties={
            "forms": SchemaProperty(
    ...
    try:
        context = util.prompt_loader.get_context("language_forms", "thai/verb")
        prompt = util.prompt_loader.get_prompt("language_forms", "thai/verb").format(
            verb=verb,
        ...
        logger.error(f"Error querying Thai verb forms for '{verb}': {e}")
        return {}, False
