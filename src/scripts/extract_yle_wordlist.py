...
Output JSON:
    data/cambridge/yle_wordlist.json
...
# Left/right x-bounds of the four columns on the alphabetic wordlist pages.
# pdfplumber's extract_text() reads a page row-by-row across all columns, so a
# row whose first cell has no POS token ("kilometre", whose "n" sits on the
# previous wrapped line) runs into the next column's entry and yields
# "kilometre king n" as one word. Reading each column separately keeps the
# entries apart; the POS-based splitting below then only has to deal with
# genuine multi-entry cells.
COLUMN_BOUNDS: Tuple[Tuple[int, int], ...] = ((60, 190), (190, 315), (315, 440), (440, 570))

# Words on the same row are grouped by their rounded `top` coordinate; two
# words differing by less than this are on the same visual line.
...
HEADER_LINES = {
    ...
    "Grammatical key",
    # The key's heading and its rows wrap across column cells, leaving
    # fragments like "Grammatical", "key" and "marker pron pronoun" standing
    # alone at the head of a column.
    "Grammatical",
    "key",
    "marker",
    "wordlist",
...
# Expansions used in the "Grammatical key" table (adj adjective, pron pronoun).
# A cell that is only key vocabulary is part of that table, not an entry.
POS_EXPANSIONS: Set[str] = {
    "adjective",
    "adverb",
    "conjunction",
    "determiner",
...
def _strip_footer_and_headers(lines: List[str]) -> List[str]:
    ...
    for ln in lines:
        ...
        if any(r.search(s) for r in FOOTER_RES):
            continue
        # A cell built only from grammatical-key vocabulary ("marker pron
        # pronoun", "int interrogative") is part of that table. A bare POS on
        # its own is not: that is an entry's POS wrapped onto the next line
        # ("bat (as sports equipment)" / "n"), so it must survive.
        tokens = s.split()
        if (
            tokens
            and any(t in POS_EXPANSIONS for t in tokens)
            and all(t in POS_EXPANSIONS or t in VALID_POS for t in tokens)
        ):
            continue
        out.append(s)
    return out
...
def _iter_entries_in_text(text: str) -> List[str]:
    ...
    The caller feeds this one column's text at a time (see `_column_lines`), so
    the stream here is already free of the cross-column welds that row-by-row
    extraction produces. A single cell can still hold more than one entry when
    a wrapped line pushes two together, which is what the POS split handles.
    """
...
def _parse_entry(raw: str) -> Optional[Entry]:
    ...
    sense: Optional[str] = None
    base_word = word_part
    # The parenthetical itself may contain parentheses -- "candy (UK sweet(s))" --
    # so match one level of nesting rather than any run of non-")".
...
def _page_columns(page: "pdfplumber.page.Page") -> List[List[str]]:
    """Return each column's lines, top to bottom, for one page.

    Words are bucketed into rows by their ``top`` coordinate and then into the
    four columns of :data:`COLUMN_BOUNDS` by ``x0``. Keeping the columns apart
    is what stops two of them being welded into one entry; see the note on
    :data:`COLUMN_BOUNDS`.
    """
    rows: Dict[int, List[Dict[str, Any]]] = defaultdict(list)
    for word in page.extract_words():
        rows[round(word["top"] / ROW_TOLERANCE)].append(word)

    columns: List[List[str]] = []
    for left, right in COLUMN_BOUNDS:
        lines: List[str] = []
        for row_key in sorted(rows):
            cell = [word for word in rows[row_key] if left <= word["x0"] < right]
            if cell:
                lines.append(" ".join(w["text"] for w in sorted(cell, key=lambda w: w["x0"])))
...
def parse_level_pages(pdf: "pdfplumber.PDF", level: str) -> List[Entry]:
    ...
    # An entry can wrap onto the next line within its column, so each column is
    # parsed as its own stream: joining the columns would re-create exactly the
    # cross-column welds that reading them separately is meant to avoid. The
    # columns are taken in reading order so a wrap at the foot of one column
    # still meets its continuation at the head of the next.
    lines: List[str] = []
    for page_idx in range(start, end + 1):
        for column in _page_columns(pdf.pages[page_idx]):
            column_cells = _strip_footer_and_headers(column)
            for index, cell in enumerate(column_cells):
                lines.append(cell)
                # A cell that closes a parenthetical but carries no POS token
                # ("a.m. (for time)", "bat (as sports equipment)") has had its
                # POS wrapped away by the PDF. When the next line is that bare
                # POS the two belong together; otherwise the entry is POS-less
                # and must be terminated, or it swallows the entry after it.
                if not cell.endswith(")"):
                    continue
                if any(token in VALID_POS for token in cell.split()):
                    continue
                following = column_cells[index + 1] if index + 1 < len(column_cells) else ""
                if following.split() and all(t in VALID_POS or t == "+" for t in following.split()):
                    continue
                lines.append("")
            # Terminate the column so a trailing fragment cannot run into it.
            lines.append("")

    # Remove single-letter section headers, keeping the column terminators.
    ...
    for ln in lines:
        if re.match(
            ...
            continue
        cleaned.append(ln)
    # Parse each column as its own stream. `_strip_footer_and_headers` drops the
    # blank column terminators, so the split is done over `lines` before that.
    raw_entries: List[str] = []
    column_lines: List[str] = []
    for ln in [*cleaned, ""]:
        if ln:
            column_lines.append(ln)
            continue
        if column_lines:
            raw_entries.extend(_iter_entries_in_text(" ".join(column_lines)))
            column_lines = []
...
def parse_thematic_pages(pdf: "pdfplumber.PDF") -> Dict[str, Dict[str, List[str]]]:
    ...
    for page_idx in range(start, end + 1):
        ...
        for row in rows:
            ...
            # Named apart from the `by_level` of word lists elsewhere in this
            # module: this one holds token groups, not finished words.
            token_groups_by_level: Dict[str, List[List[dict]]] = {
                "starters": [],
                "movers": [],
                "flyers": [],
            }
            for t in data_tokens:
                x = t["x0"]
                if x < mid_sm:
                    token_level = "starters"
                elif x < mid_mf:
                    token_level = "movers"
                else:
                    token_level = "flyers"
                bucket = token_groups_by_level[token_level]
                if bucket and (t["x0"] - (bucket[-1][-1]["x0"] + bucket[-1][-1]["width"])) < 15:
                    bucket[-1].append(t)
                else:
                    bucket.append([t])
            for token_level, groups in token_groups_by_level.items():
                for grp in groups:
                    theme_map[current_theme][token_level].append(" ".join(g["text"] for g in grp))
    ...
    for k, by_level in theme_map.items():
        tgt = redirect[k]
        empty_levels: Dict[str, List[str]] = {"starters": [], "movers": [], "flyers": []}
        dest: Dict[str, List[str]] = merged.setdefault(tgt, empty_levels)
        for level_name, level_words in by_level.items():
            dest[level_name].extend(level_words)
