#!/usr/bin/env python3
"""Command-line interface for sentence generation.

Generates example sentences for vocabulary words, using either:
...
Examples:
  ...
  python -m agents.buivolas --task generate-sentences --mode guided --level 3 --num-sentences 5 --limit 10
    ...
    parser.add_argument(
        "--level",
        type=int,
        help="Generate LLM sentences for lemmas at a specific difficulty level (1-199, for llm mode)",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def _get_llm_lemmas(service: SentenceGenerationService, args: argparse.Namespace) -> list[Lemma]:
...
def enqueue_generation_work(service: SentenceGenerationService, args: argparse.Namespace) -> int:
    """Discover sentence-generation work and enqueue capability-named tasks."""
    session = service.get_session()
    try:
        if args.task == "generate-candidates":
            pattern_ids = sorted(args.patterns or [])
            identity = "all" if args.all_patterns else ",".join(pattern_ids)
            dedup_hash = hashlib.sha256(identity.encode()).hexdigest()[:12]
            if not args.dry_run:
                result = enqueue_task(
                    session,
                    task_type=TaskType.SENTENCES_PATTERNS_GENERATE,
                    ...
                    payload={
                        ...
                        "source_component": "agents.buivolas",
                    },
                    dedup_key=(
                        f"{TaskType.SENTENCES_PATTERNS_GENERATE}:{dedup_hash}:"
                        f"{args.limit or 'all'}"
                    ),
                )
                session.commit()
                logger.info(
                    "%s pattern-generation task %s",
                    "Queued" if result.created else "Reused",
                    result.task.id,
                )
            else:
                logger.info("Would queue pattern generation for %s", identity)
            return 0

        lemmas = _get_llm_lemmas(service, args)
        if not lemmas:
            logger.error("No supported lemmas found for sentence generation")
            return 1

        enqueued_count = 0
        for lemma in lemmas:
            if args.dry_run:
                logger.info("Would queue %s sentence generation for %s", args.mode, lemma.guid)
                enqueued_count += 1
                continue
            result = enqueue_task(
                session,
                task_type=TaskType.SENTENCES_EXAMPLES_GENERATE,
                target_type="lemma",
                target_id=lemma.id,
                payload={
                    "schema_version": 1,
                    ...
                    "max_combinations": args.pattern_limit,
                    "difficulty_context": args.level,
                    "max_vocabulary_level": 7,
                    "model": service.config.model,
                    "source_component": "agents.buivolas",
                },
                dedup_key=f"{TaskType.SENTENCES_EXAMPLES_GENERATE}:{lemma.id}:{args.mode}",
            )
            if result.created:
                enqueued_count += 1
        session.commit()
        logger.info("Queued %s sentence-generation task(s)", enqueued_count)
        return 0
    finally:
        session.close()
...
def main() -> int:
    ...
    service = SentenceGenerationService(config=config, dry_run=args.dry_run)

    if not args.execute_inline:
        return enqueue_generation_work(service, args)

    if args.task == "generate-candidates":
        pattern_dict = {p["pattern_id"]: p for p in SIMPLE_PATTERNS}

        if args.all_patterns:
            results = service.pattern_generator.generate_candidates_all_patterns(
            ...
            for pattern in patterns_to_generate:
                result = service.pattern_generator.generate_candidates_for_pattern(
        ...
        if args.mode == "pattern":
            ...
            result = service.generate_pattern_sentences_for_guid(
            ...
            for i, lemma in enumerate(lemmas, 1):
                ...
                result = service.generate_guided_sentences_for_lemma(
                    lemma=lemma,
                    num_sentences=args.num_sentences,
                    max_vocabulary_level=7,
                )
                ...
                if result.get("success") and result.get("sentences"):
                    ...
                    if not args.dry_run:
                        session = service.get_session()
                        try:
                            store_result = service.store_guided_sentences(
            ...
            for i, lemma in enumerate(lemmas, 1):
                ...
                result = service.generate_llm_sentences_for_lemma(
                    lemma=lemma,
                    num_sentences=args.num_sentences,
                ...
                if result.get("success") and result.get("sentences"):
                    ...
                    if not args.dry_run:
                        session = service.get_session()
                        try:
                            store_result = service.store_llm_sentences(
                                sentences_data=sentences,
                                source_lemma=lemma,
                                session=session,
                            )
