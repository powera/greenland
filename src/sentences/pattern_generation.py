"""Pattern-based sentence generation and persistence."""
...
class PatternSentenceGenerator:
    ...
    def generate_candidates_for_pattern(
        ...
        working_session = session if session is not None else self.get_session()
        try:
            combinations = self.generate_all_combinations(
                working_session, pattern, max_combinations
            )
            ...
            for i, combo in enumerate(combinations, 1):
                ...
                result = self.save_candidate_sentence(
                    working_session, pattern, combo, template_text
                )
    ...
    def generate_candidates_for_guid(
        ...
        working_session = session if session is not None else self.get_session()
        try:
            lemma = working_session.query(Lemma).filter(Lemma.guid == guid).first()
            if not lemma:
            ...
            for pattern in ALL_PATTERNS:
                if self.generate_combinations_for_lemma(
                    working_session, pattern, lemma, max_combinations=1
                ):
            ...
            for pattern in eligible_patterns:
                combinations = self.generate_combinations_for_lemma(
                    working_session, pattern, lemma, max_combinations=max_combinations
                )
    ...
    def generate_candidates_all_patterns(
        ...
        for pattern in ALL_PATTERNS:
            result = self.generate_candidates_for_pattern(
                pattern,
                max_combinations=max_per_pattern,
                session=session,
            )
