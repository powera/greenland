"""Command-line interface for vocabulary-driven conversation generation.
...
from sentences.conversation_planning import (
...
"Sarka" means "magpie" in Lithuanian - known for being talkative and social.
    ...
    parser.add_argument(
        "--execute-inline",
        action="store_true",
        help="Execute LLM work immediately instead of enqueueing it (debugging only)",
    )

    return parser


def show_level_words(agent: ConversationPlanner, level: int) -> None:
...
def enqueue_level_work(
    ...
    Args:
        agent: Conversation planner instance
...
def get_conversation_queue_stats(session: Any) -> Dict[str, int]:
    """Get statistics for vocabulary-driven conversation tasks."""
...
# Compatibility name retained for callers of the former agent CLI.
get_sarka_queue_stats = get_conversation_queue_stats
...
def main() -> None:
    ...
    planner = ConversationPlanner(config=config)

    # VIEW MODE: Display a specific conversation
    if args.view:
        conversation = planner.get_conversation_details(args.view)
        if not conversation:
    ...
    if args.show_words:
        ...
        if args.level_end:
            for level in range(args.level, args.level_end + 1):
                show_level_words(planner, level)
                print()
        else:
            show_level_words(planner, args.level)
        return
    ...
    if args.definitions:
        ...
        for current_level in levels:
            ...
            result = planner.generate_definitions_for_level(
                level=current_level,
                max_level=args.max_level,
                category=args.category,
                max_word_usage=args.max_word_usage,
    ...
    if args.generate:
        ...
        if use_new_options:
            ...
            result = planner.generate_with_options(
                max_level=args.max_level,
                level=args.level,
                category=args.category,
            ...
            for level in levels:
                ...
                result = planner.generate_for_level(
                    level=level,
                    num_conversations=args.num_conversations,
                    num_sentences=args.num_sentences,
                    dry_run=args.dry_run,
