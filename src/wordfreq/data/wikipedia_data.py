...
def process_article(wiki_loader: WikiLoader, article_name: str) -> Counter:
    ...
    try:
        ...
        print(f"Processed article: {article_name} - {len(tokens)} tokens, {len(word_counts)} unique words")
        return word_counts
...
def main():
    ...
    article_names = [item for sublist in wordfreq.data.vital1000.vital.values() if isinstance(sublist, list) for item in sublist]
