...
class LinguisticReviewer:
    ...
    def display_word_info(self, word_text: str) -> bool:
        ...
        if frequencies:
            ...
            for freq in frequencies:
                ...
                freq_info = (
                    f", frequency {freq.frequency:.6f}" if freq.frequency is not None else ""
                )
        ...
        if not derivative_forms:
            ...
            for i, derivative_form in enumerate(derivative_forms):
                verified_str = (
                    f" {self.c.GREEN}[✓]{self.c.ENDC}" if derivative_form.verified else ""
                )
                ...
                print(
                    f"    {self.c.BLUE}Part of speech:{self.c.ENDC} {derivative_form.lemma.pos_type}"
                )
                print(
                    f"    {self.c.BLUE}Grammatical form:{self.c.ENDC} {derivative_form.grammatical_form}"
                )
                if derivative_form.lemma.pos_subtype:
                    print(
                        f"    {self.c.BLUE}Subtype:{self.c.ENDC} {derivative_form.lemma.pos_subtype}"
                    )
                ...
                if derivative_form.phonetic_pronunciation:
                    print(
                        f"    {self.c.BLUE}Phonetic Pronunciation:{self.c.ENDC} {derivative_form.phonetic_pronunciation}"
                    )
    ...
    def list_problematic_words(self, limit: int = 10) -> None:
        ...
        for item in words:
            ...
            unverified_count = sum(
                1 for definition in item["definitions"] if not definition.get("verified", True)
            )
            ...
            for i, definition in enumerate(item["definitions"]):
                verified_str = (
                    f" {self.c.GREEN}[✓]{self.c.ENDC}"
                    if definition.get("verified", False)
                    else f" {self.c.RED}[✗]{self.c.ENDC}"
                )
    ...
    def list_common_words_by_pos_subtype(
        self, pos_type: str, pos_subtype: str, limit: int = 20
    ) -> None:
        """
        List the most common words for a specified part of speech.
        ...
        words = linguistic_db.get_common_words_by_pos(
            self.session, pos_type, pos_subtype=pos_subtype, limit=limit
        )
        if not words:
            print(
                f"{self.c.YELLOW}No words found with part of speech '{pos_type}/{pos_subtype}'.{self.c.ENDC}"
            )
            return
    ...
    def find_missing_data(self, limit: int = 10) -> None:
        ...
        for word in words:
            print(
                f"{self.c.CYAN}{word.token}{self.c.ENDC} (rank: {word.frequency_rank}) - Missing: derivative forms"
            )
    ...
    def show_word_list_access_info(self) -> None:
        ...
        print(
            f'  {self.c.GREEN}python -m src.wordfreq.tools.word_categorizer "category_name"{self.c.ENDC}'
        )

        print(f"\n{self.c.BLUE}Examples:{self.c.ENDC}")
        print(f"  {self.c.CYAN}# Categorize top 1000 words as animals{self.c.ENDC}")
        print(
            f'  {self.c.GREEN}python -m src.wordfreq.tools.word_categorizer "animals"{self.c.ENDC}'
        )

        print(f"  {self.c.CYAN}# Find past participles in top 500 words{self.c.ENDC}")
        print(
            f'  {self.c.GREEN}python -m src.wordfreq.tools.word_categorizer "past participles" --limit 500{self.c.ENDC}'
        )

        print(f"  {self.c.CYAN}# Find words similar to 'shiny' and save results{self.c.ENDC}")
        print(
            f'  {self.c.GREEN}python -m src.wordfreq.tools.word_categorizer "words like shiny" --save-json results.json{self.c.ENDC}'
        )
        ...
        while True:
            ...
            try:
                ...
                if choice == 0:
                    ...
                    percent_with_defs = (
                        stats["tokens_with_derivative_forms"] / stats["total_word_tokens"] * 100
                        if stats["total_word_tokens"] > 0
                        else 0
                    )
                    print(
                        f"  Tokens with derivative forms: {stats['tokens_with_derivative_forms']} ({percent_with_defs:.1f}%)"
                    )
                    ...
                    pos_type = (
                        input(
                            f"{self.c.BOLD}Enter part of speech (noun, verb, adjective, etc.): {self.c.ENDC}"
                        )
                        .strip()
                        .lower()
                    )
