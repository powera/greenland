...
def get_german_noun_lemmas(db_path: str, limit: int = None) -> List[Dict]:
...
def process_lemma_forms(client: LinguisticClient, lemma_id: int, db_path: str) -> bool:
    """Process and store German noun forms for a lemma."""
    session = get_session(db_path)
    try:
        ...
        if sum(1 for f in existing_forms if f.grammatical_form in [g.value for g in FORM_MAPPING.values()]) >= 2:
            logger.info(f"Lemma ID {lemma_id} already has German forms, skipping")
            return True

        forms_dict, success = client.query_german_noun_forms(lemma_id)
...
def main():
    parser = argparse.ArgumentParser(description="Generate German noun forms")
    ...
    lemmas = get_german_noun_lemmas(args.db_path, limit=args.limit)
    logger.info(f"Found {len(lemmas)} German noun lemmas")
    if not lemmas:
        return
