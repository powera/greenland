...
def get_french_noun_lemmas(db_path: str, limit: int = None) -> List[Dict]:
...
def process_lemma_forms(client: LinguisticClient, lemma_id: int, db_path: str) -> bool:
    ...
    try:
        ...
        if sum(1 for f in existing_forms if f.grammatical_form in [g.value for g in FORM_MAPPING.values()]) >= 2:
            logger.info(f"Lemma ID {lemma_id} already has French forms, skipping")
            return True

        forms_dict, success = client.query_french_noun_forms(lemma_id)
        ...
        for form_name, form_text in forms_dict.items():
            ...
            session.add(linguistic_db.DerivativeForm(
                ...
                is_base_form=(form_name == "singular"), verified=False))
            stored += 1
        session.commit()
...
def main():
    parser = argparse.ArgumentParser(description="Generate French noun forms")
    ...
    lemmas = get_french_noun_lemmas(args.db_path, limit=args.limit)
    logger.info(f"Found {len(lemmas)} French noun lemmas")
    if not lemmas:
        return
