...
class WordProcessor:
    ...
    def process_all_words(
        self, limit: Optional[int] = None, skip_processed: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if skip_processed:
            ...
            # Get word tokens in combined-rank order (across every enabled
            # corpus and tier source), most frequent first.
            words = linguistic_db.get_word_tokens_by_combined_frequency_rank(
                session, limit=limit or 100000
            )
        ...
        if total_words == 0:
            return {"total": 0, "processed": 0, "successful": 0, "batches": 0}
        ...
        for i in range(0, total_words, self.batch_size):
            ...
            logger.info(
                f"Overall progress: {processed}/{total_words} word tokens processed ({successful} successful)"
            )
