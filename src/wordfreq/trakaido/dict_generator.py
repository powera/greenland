...
def get_lithuanian_word_for_lemma(session: Any, lemma: Lemma) -> str:
    ...
    if lithuanian_translation:
        return lithuanian_translation
...
def get_chinese_word_for_lemma(
    ...
    """
    # Get from the translation table (stored as Traditional)
...
def generate_structure_data(
    ...
    for subtype in subtypes:
        lemmas = get_lemmas_by_subtype_and_level(
            session, pos_subtype=subtype, difficulty_level=difficulty_level, lang=lang
        )
        ...
        if lemmas:
            ...
            for lemma in lemmas:
                if lemma.guid:
                    if validate_guid_as_variable_name(lemma.guid):
                        ...
                        print(
                            f"Warning: Skipping GUID '{lemma.guid}' in structure (not a valid Python identifier)"
                        )
                        skipped_guids += 1
            ...
            if guid_list:
                ...
                if skipped_guids > 0:
                    print(
                        f"Skipped {skipped_guids} invalid GUIDs in {display_subtype} for level {difficulty_level}"
                    )
...
def generate_structure_file(
    ...
    if not level_data:
        print(
            f"No data found for difficulty level {difficulty_level}, generating empty structure file"
        )
...
def get_difficulty_levels_with_data(session: Any) -> List[int]:
    ...
    levels = (
        session.query(Lemma.difficulty_level.distinct())
...
def main() -> None:
    ...
    parser.add_argument(
        "--language",
        choices=["lithuanian", "chinese"],
        default="lithuanian",
        help="Choose language - lithuanian or chinese",
    )
    parser.add_argument(
        "--level", "-l", type=int, help="Generate structure file for specific difficulty level only"
    )
    parser.add_argument(
        "--subtype", "-s", help="Generate dictionary file for specific subtype only"
    )
    parser.add_argument(
        "--type",
        "-t",
        choices=["structure", "dictionary", "both"],
        default="both",
    ...
    try:
        ...
        if args.subtype and args.type in ["dictionary", "both"]:
            ...
            filepath = generate_dictionary_file(
                session, args.subtype, output_dir, lang=args.language
            )
            if filepath:
                generated_files.append(filepath)
                print(f"Generated: {filepath}")
        ...
        if not args.level and not args.subtype:
            ...
            if args.type == "structure":
                generated_files = generate_all_structure_files(
                    session, output_dir, lang=args.language
                )
            elif args.type == "dictionary":
                generated_files = generate_all_dictionary_files(
                    session, output_dir, lang=args.language
                )
            else:  # both
