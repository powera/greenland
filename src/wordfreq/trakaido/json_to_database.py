...
def load_trakaido_json(json_path: str) -> List[Dict[str, Any]]:
    ...
    for i, entry in enumerate(data):
        ...
        if pos_type not in valid_pos_types:
            raise ValueError(
                f"Entry {i} has invalid POS type '{entry['POS']}'. Valid types: {', '.join(sorted(valid_pos_types))}"
            )
...
def get_pos_and_subtype_for_category(display_category_name: str) -> tuple[str, Optional[str]]:
    ...
    if display_category_name not in category_mapping:
        print(
            f"WARNING: No category mapping found for '{display_category_name}', using default (noun, other)"
        )
...
def find_or_create_lemma(
    ...
    if existing_lemma:
        ...
        if update_difficulty and existing_lemma.difficulty_level != difficulty_level:
            ...
            session.commit()
            print(
                f"    Updated difficulty level for '{english_word}' from {old_difficulty} to {difficulty_level}"
            )
    ...
    if english_word in english_alternative_map:
        ...
        for alternative in alternatives:
            ...
            try:
                ...
                freq_info = (
                    f", freq_rank: {alternative_word_token.frequency_rank}"
                    if alternative_word_token and alternative_word_token.frequency_rank
                    else ""
                )
                ...
                print(
                    f"      ❌ Failed to create alternative '{alternative}' for {english_word}: {e}"
                )
    ...
    for i, entry in enumerate(trakaido_data):
        ...
        if lemma:
            ...
            if verbose:
                ...
                pos_info = (
                    f", POS: {lemma.pos_type}/{lemma.pos_subtype}"
                    if lemma.pos_subtype
                    else f", POS: {lemma.pos_type}"
                )
                print(
                    f"    ✅ {english_word} -> {lithuanian_word} (GUID: {lemma.guid}{pos_info}{freq_info}{notes_info})"
                )
