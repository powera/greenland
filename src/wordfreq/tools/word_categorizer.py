...
class WordCategorizer:
    ...
    def __init__(
        self, db_path: str = constants.WORDFREQ_DB_PATH, model: str = constants.DEFAULT_MODEL
    ):
        """
    ...
    def categorize_words(self, category: str, word_limit: int = 1000) -> CategorizationResult:
        ...
        word_match_schema = Schema(
            ...
            "A word that matches the category",
            {
                ...
                "is_primary_meaning": SchemaProperty(
                    "boolean", "Whether this represents the primary meaning of the word"
                ),
                "explanation": SchemaProperty(
                    "string", "Brief explanation of why this word fits the category"
                ),
            },
        )
        ...
        response_schema = Schema(
            ...
            "Response containing categorized words",
            {
                ...
                "matching_words": SchemaProperty(
                    "array",
                    "List of words that match the category",
                    array_items_schema=word_match_schema,
                ),
        ...
        try:
            ...
            logger.info(
                f"Categorization complete: {result.total_matches} matches out of {result.total_analyzed} words"
            )
            return result
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Categorize words from frequency lists using LLM analysis"
    )
    parser.add_argument(
        "category", help="Category to match words against (e.g., 'animals', 'past participles')"
    )
    parser.add_argument(
        "--limit", type=int, default=1000, help="Number of top words to analyze (default: 1000)"
    )
    parser.add_argument("--model", default="claude-3-5-sonnet-20241022", help="LLM model to use")
    parser.add_argument(
        "--no-explanations", action="store_true", help="Hide explanations in output"
    )
    ...
    try:
        ...
        result = categorizer.categorize_words(category=args.category, word_limit=args.limit)
