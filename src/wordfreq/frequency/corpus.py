...
class CorpusConfig:
    ...
    language_code: str = (
        "en"  # Language code for the words in this corpus (e.g., "en", "lt", "zh", "fr")
    )
    file_type: str = "json"  # File type (json, subtlex)
    value_type: str = "auto"  # How to interpret values: "rank", "frequency", or "auto"
    corpus_weight: float = (
        1.0  # Overall weight of this corpus in calculations (0.0 = exclude, 1.0 = full weight)
    )
    max_unknown_rank: Optional[int] = (
        None  # Maximum rank for unknown words (None = use corpus size)
    )
    enabled: bool = True
...
def validate_corpus_configs() -> List[str]:
    ...
    for config in CORPUS_CONFIGS:
        ...
        if not 0.0 <= config.corpus_weight <= 1.0:
            errors.append(
                f"Invalid corpus_weight for {config.name}: {config.corpus_weight} (must be 0.0-1.0)"
            )

        # Validate max unknown rank
        if config.max_unknown_rank is not None and config.max_unknown_rank <= 0:
            errors.append(
                f"Invalid max_unknown_rank for {config.name}: {config.max_unknown_rank} (must be positive)"
            )
...
def sync_corpus_configs_to_db(
    ...
    try:
        ...
        logger.info(
            f"Corpus sync completed: {added_count} added, {updated_count} updated, {disabled_count} disabled"
        )
        ...
        return {"success": False, "errors": [str(e)], "added": 0, "updated": 0, "disabled": 0}
    finally:
        if should_close:
            session.close()
...
def get_corpus_size(
    corpus_name: str, session: Optional[Session] = None, db_path: Optional[str] = None
) -> int:
    """
