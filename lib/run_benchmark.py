...
def run_0030_analyze_paragraph(model):
    ...
    for question_row in sentence_list:
        ...
        # Clean up query by removing any trailing "Answer: " prompt
        ...
        prompt = f"""
Analyze this passage and question carefully: {query}
...
What is the subject for the sentence where the location is {correct['location']}?
...
Respond in JSON with a 'subject' field containing only the subject's name."""
        ...
        try:
            response = json.loads(response_unparsed)
            is_correct = response["subject"].lower() == correct["name"].lower()
...
"""
        ...
        is_correct = question_json["continuation"] in response
        ...
        log_result(
            ...
            is_correct,
            eval_msec=perf["total_msec"],
            debug_json=None if is_correct else json.dumps({
                "response": response,
                "expected": question_json["continuation"]
            })
        )
...
""")
    ...
    session = benchmarks.datastore.create_dev_session()
    success, msg = benchmarks.datastore.insert_run(
        session, model, "0040_general_knowledge", "correct",
        correct_answers, run_details=run_details["correct"]
    )
    if not success:
        print(msg)
